...
function MemoryGame() {
    ...
    const renderCards = () => {
        ...
        let allTurnedChecker = renderArray.every(card => card.turned === true);
        // Check if all card names "Found"
        let allFoundChecker = renderArray.every(card => card.name === "Found");
        ...
        if (allFoundChecker) {
            ...
            // Map through renderarray and make new div with index number inside for every card of array WITHOUT onClick option
            return renderArray.map((card, i) => {
                return (
                    <div className="grid-item" key={card.name + i}
                        style={card.turned ? { backgroundImage: `url(${card.url})` } : { backgroundImage: `url(${Backside})` }} />
                );
            });
        } else {
            // Map through renderarray and make new div with index number inside for every card of array WITH onClick option
            return renderArray.map((card, i) => {
                return (
                    <div className="grid-item" key={card.name + i}
                        style={card.turned ? { backgroundImage: `url(${card.url})` } : { backgroundImage: `url(${Backside})` }}
    ...
    /*
                        <div className="grid-item" key={card.name + i}>
                        <img className="image" src={card.turned ?  card.url : Backside } alt={card.name+i} onClick={() => clickCard(i)}></img>
                    </div>
                        */
    return (
        <div id="container">
